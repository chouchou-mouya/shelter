...
const weekToDay = (year) => {
  ...
  return [...Array(count).keys()].map((el) => {
    ...
    let first_day = moment().year(year).week(week_value).day("Sunday").format("YYYY/MM/DD")
    let end_day = moment().year(year).week(week_value).day("Saturday").format("YYYY/MM/DD")
    if (year === '2022') {
      first_day = moment().day("Sunday").year(year).week(week_value).format("YYYY/MM/DD")
      end_day = moment().day("Saturday").year(year).week(week_value).format("YYYY/MM/DD")
    }
    return {
